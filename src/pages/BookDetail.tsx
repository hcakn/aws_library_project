...
export function BookDetail() {
  ...
  const loadReadingLists = async () => {
    ...
    try {
      const lists = await getReadingLists(user.id);
  ...
  const handleAddToList = () => {
    if (!user) {
      alert('Please sign in to add books to reading lists');
      navigate('/login');
      return;
    }

    if (readingLists.length === 0) {
      alert('Please create a reading list first');
  ...
  const handleSelectList = async (list: ReadingList) => {
    ...
    try {
      ...
      alert(`"${book.title}" has been added to "${list.name}"!`);
      setShowListModal(false);
    ...
    <div className="min-h-screen py-12 px-4">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="mt-8 glass-effect rounded-3xl shadow-xl border border-white/20 p-8 md:p-12">
          <div className="flex justify-between items-center mb-6">
            ...
            <Button variant="primary" size="sm" onClick={handleWriteReview}>
              Write Review
            </Button>
          </div>
          ...
          {reviews.length === 0 ? (
            ...
            <div className="space-y-6">
              {reviews.map((review) => (
                <div
                  key={review.id}
                  className="border border-slate-200 rounded-xl p-6 hover:border-violet-300 transition-colors"
                >
      ...
      {showListModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-effect rounded-3xl shadow-2xl border border-white/20 max-w-md w-full p-8">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-slate-900">Select a Reading List</h3>
              <button
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {readingLists.map((list) => (
                <button
                  ...
                  className="w-full text-left p-4 rounded-xl border border-slate-200 hover:border-violet-400 hover:bg-violet-50 transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-600 mt-1">{list.bookIds.length} books</p>
                    </div>
                    <svg
      ...
      {showReviewModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-effect rounded-3xl shadow-2xl border border-white/20 max-w-lg w-full p-8">
            <div className="flex justify-between items-center mb-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 min-h-[120px] resize-none"
                />
              </div>
